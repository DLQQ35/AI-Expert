...
print(f"{Fore.CYAN} 🐍Welcome to Sentiment Spy's Upgraded Version!🐍{Style.RESET_ALL}")
...
print(f"Type a sentence and I will analyze its sentiment through my enhanced magical powers!")
...
while True:
    ...
    if not user_input:
        print(f"{Fore.RED}Please enter a sentence to help me analyze through my enhanced magical powers.{Style.RESET_ALL}")
        continue
        ...
        if not conversation_history:
            ...
            for i, (text, polarity, sentiment_type) in enumerate(conversation_history, start=1):
                if sentiment_type == "positive":
                    color = Fore.GREEN
                    emoji = "😆"
                    que = input("Why are you happy?")
                    print("That's great to hear!")
                elif sentiment_type == "negative":
                    color = Fore.RED
                    emoji = "😭"
                    que = input("Why are you sad?")
                    print("I'm sorry to hear that.")
                else:
                    color = Fore.YELLOW
                    emoji = "😐"
                    que = input("What's making you neutral?")
                    print("I see.")
                print("I guess the reason is gonna be a mystery cause i dont have the power to know that! Gonna learn that magic and then know your mystery.")
                print(f" {i}. {color}{emoji} {text} {que}" f"Polarity: {polarity:.2f}, {sentiment_type}{Style.RESET_ALL}")
        continue
